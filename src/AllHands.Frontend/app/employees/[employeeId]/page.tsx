...
type EmployeeDetails = { id: string; status: EmployeeStatus };
...
export default function EmployeeById() {
  ...
  const statusDisabled = disabled || form.status === "Unactivated";
  ...
  const loadEmployee = async (idFromRoute: string) => {
    ...
    try {
      ...
      const status = (toStr(pick(dto, "status", "Status")) || "Undefined") as EmployeeStatus;
      const managerId = toStr(
        pick(dto, "managerId", "ManagerId") ?? pick(dto?.manager, "employeeId", "EmployeeId", "id", "Id") ?? pick(dto?.Manager, "employeeId", "EmployeeId", "id", "Id")
      );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (employee.status !== form.status && employee.status !== "Unactivated") {
        if (form.status === "Fired") {
      ...
      <div className="pageWrapper">
        <div className="pageCard">
          <div style={{ marginBottom: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <h1 className="profileTitle">{employee ? `${form.firstName} ${form.lastName}` : "Employee info"}</h1>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div className="accRow">
              ...
              <input className="accInput" value={form.firstName} onChange={(e) => setForm((p) => ({ ...p, firstName: e.target.value }))} disabled={disabled} />
            </div>
            <div className="accRow">
              <label className="accLable">Middle name</label>
              <input className="accInput" value={form.middleName} onChange={(e) => setForm((p) => ({ ...p, middleName: e.target.value }))} disabled={disabled} />
            </div>
            <div className="accRow">
              <label className="accLable">Last name</label>
              <input className="accInput" value={form.lastName} onChange={(e) => setForm((p) => ({ ...p, lastName: e.target.value }))} disabled={disabled} />
            </div>
            <div className="accRow">
              <label className="accLable">Email</label>
              <input className="accInput" type="email" value={form.email} onChange={(e) => setForm((p) => ({ ...p, email: e.target.value }))} disabled={disabled} />
            </div>
            <div className="accRow">
              <label className="accLable">Phone</label>
              <input className="accInput" value={form.phoneNumber} onChange={(e) => setForm((p) => ({ ...p, phoneNumber: e.target.value }))} disabled={disabled} />
            </div>
            <div className="accRow">
              <label className="accLable">Start date</label>
              <input className="accInput" type="date" value={form.workStartDate} onChange={(e) => setForm((p) => ({ ...p, workStartDate: e.target.value }))} disabled={disabled} />
            </div>
            <div className="accRow">
              <label className="accLable">Manager</label>
              <select className="accInput" value={form.managerId} onChange={(e) => setForm((p) => ({ ...p, managerId: e.target.value }))} disabled={disabled}>
                <option value="">Not set</option>
                {managers.map((m) => (<option key={m.id} value={m.id}>{formatFullName(m)}</option>))}
              </select>
            </div>
            <div className="accRow">
              <label className="accLable">Position</label>
              <select className="accInput" value={form.positionId} onChange={(e) => setForm((p) => ({ ...p, positionId: e.target.value }))} disabled={disabled}>
                <option value="">Not set</option>
            ...
            <div className="accRow">
              ...
              <select className="accInput" value={form.roleId} onChange={(e) => setForm((p) => ({ ...p, roleId: e.target.value }))} disabled={disabled}>
                <option value="">Not set</option>
            ...
            <div className="accRow">
              ...
              <select className="accInput" value={form.status} onChange={(e) => setForm((p) => ({ ...p, status: e.target.value as EmployeeStatus }))} disabled={statusDisabled}>
            ...
            <div className="profileButtons" style={{ marginTop: 24, display: "flex", gap: 16 }}>
              {canEditEmployee && (<button className="profileButtonPrimary" onClick={handleSave} disabled={disabled || !employee?.id}>{saving ? "Saving..." : "Save changes"}</button>)}
              {canDeleteEmployee && (
                <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start" }}>
                  ...
                  {showDeleteHint && (<div style={{ marginTop: 10, fontSize: 14, opacity: 0.85 }}>Are you sure? Click the button again within a few seconds to confirm deletion.</div>)}
                </div>
              )}
            </div>
          </div>
