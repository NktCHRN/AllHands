...
public sealed class BatchTimeOffBalanceUpdater(IDocumentStore documentStore, IOptions<TimeOffBalanceAutoUpdaterOptions> options, ILogger<BatchTimeOffBalanceUpdater> logger) : IBatchTimeOffBalanceUpdater
{
    ...
    private async Task UpdateInCompanyAsync(IQuerySession querySession, Guid companyId, CancellationToken cancellationToken)
    {
        ...
        var employeesCount = await querySession.Query<Employee>()
            .Where(x => x.TenantIsOneOf(company.Id.ToString()) && x.WorkStartDate < currentMonthStart && x.Status != EmployeeStatusConstants.Fired)
        ...
        for (var skip = 0; skip < employeesCount; skip += batchSize)
        {
            ...
            var employees = await documentSession.Query<Employee>()
                .Where(x => x.WorkStartDate < currentMonthStart && x.Status != EmployeeStatusConstants.Fired)
                .OrderBy(x => x.Id)
                .Skip(skip)
                .Take(batchSize)
                .ToListAsync(cancellationToken);
